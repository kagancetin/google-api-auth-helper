import { GmailTokenHelper } from '../src/index';

describe('GmailTokenHelper', () => {
  const mockConfig = {
  ...
  test('should initialize correctly with provided config', () => {
    const helper = new GmailTokenHelper(mockConfig);
 ...
 test('should include custom scopes in the URL', () => {
  ...
  const customHelper = new GmailTokenHelper({ ...mockConfig, scopes: customScope });
